...
public class ControllerNewAccount {
	...
	*/
	...
	protected static void doCreateUser() {
		...
		if (inputValidation.usernameRecognizer.usernameValidate(ViewNewAccount.text_Username.getText())
			...
			if (UpdatePwdInvalid()) {
				...
				// The Password is invalid, so explain what is incorrect by
				// using check marks to indicate what has and has not been found
				// the listed errors are:
				//
				ViewNewAccount.alertPasswordInvalidError.showAndWait();
			}
		} else {
			// The Username is invalid, so clear username and explain that
			// usernames must NOT contain special symbols, spaces, and length
			// must be greater than or equal to 4 and less than or equal to 32
			ViewNewAccount.text_Username.setText("");
			ViewNewAccount.alertUsernameInvalidError.showAndWait();
		}
	}
	...
	protected static boolean UpdatePwdInvalid() {
		...
		String PwdError = inputValidation.PasswordValidation.passwordEvaluator(password);
		...
		ViewNewAccount.alertPasswordInvalidError.setTitle("Invalid Password");

		TextFlow headerFlow = new TextFlow();
		Text first = new Text("Here are the current errors identified:\n\n");
		headerFlow.getChildren().add(first);

		for (int i = 0; i < messages.length; i++) {
			boolean ok = ((bitmap >> i) & 1) == 1;
			Text line = new Text((ok ? "❌ " : "✅ ") + messages[i] + "\n");
			line.setStyle(ok ? "-fx-fill: red;" : "-fx-fill: green;");
			headerFlow.getChildren().add(line);
		}

		//
		ViewNewAccount.alertPasswordInvalidError.getDialogPane().setHeader(headerFlow);
		ViewNewAccount.alertPasswordInvalidError.getDialogPane().setContentText("Correct password and try again.");

		return (PwdError.length() == 0);
